...
abbrev AssocList' (α : Type u) (β : Type v) := AssocList α (fun _ => β)
...
theorem findEntry?_cons [BEq α] {l : AssocList α β} {k a : α} {v : β k} :
    (l.cons k v).findEntry? a = bif k == a then some ⟨k, v⟩ else l.findEntry? a := rfl

theorem findEntry?_cons_of_true [BEq α] {l : AssocList α β} {k a : α} {v : β k} (h : k == a) :
    (l.cons k v).findEntry? a = some ⟨k, v⟩ := by
  simp [findEntry?, h]

theorem findEntry?_cons_of_false [BEq α] {l : AssocList α β} {k a : α} {v : β k} (h : (k == a) = false) :
